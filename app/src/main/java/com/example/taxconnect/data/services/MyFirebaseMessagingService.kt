...
class MyFirebaseMessagingService : FirebaseMessagingService() {
    ...
    private fun handleDataMessage(data: Map<String, String>) {
        ...
        when (type) {
            "message" -> {
                ...
                // Launch a coroutine to fetch the image, then show notification
                kotlinx.coroutines.GlobalScope.launch {
                    val avatarBitmap = fetchBitmapFromUrl(senderImage)
                    sendMessageNotification(title, body, senderName, chatId, senderId, pendingIntent, avatarBitmap)
                }
            }
            "call" -> {
    ...
    private fun sendMessageNotification(
        ...
        // --- Mark-as-Read action ---
        val markReadIntent = Intent(this, CallActionReceiver::class.java).apply {
            action = CallActionReceiver.ACTION_MARK_READ
        ...
        val markReadPendingIntent = PendingIntent.getBroadcast(
            this, notificationId + 200, markReadIntent, markReadFlags
        )
        ...
        val replyPendingIntent = PendingIntent.getBroadcast(
            this, notificationId + 100, replyIntent, replyFlags
        )
        ...
        val replyAction = NotificationCompat.Action.Builder(
            R.drawable.ic_chat_bubble_material3,
        ...
        // --- Sender avatar: Loaded URL image OR initials drawn onto a colored bitmap ---
        val largeIcon = avatarBitmap ?: buildInitialsBitmap(senderName ?: "?", 0xFF3B82F6.toInt())

        // --- MessagingStyle (best display on Android 7+) ---
        val me = androidx.core.app.Person.Builder()
            .setName(getString(R.string.notification_me))
            .build()
        val sender = androidx.core.app.Person.Builder()
            .setName(senderName ?: getString(R.string.notification_sender_default))
        ...
        val builder = NotificationHelper.getBaseNotificationBuilder(
        ...
        NotificationHelper.showSummaryNotification(
            this, NotificationHelper.CHANNEL_MESSAGES, NotificationHelper.GROUP_MESSAGES,
            1001, getString(R.string.notification_new_messages_title),
            getString(R.string.notification_new_messages_body)
        )
    }
    ...
    private fun sendCallNotification(
        ...
        // Caller avatar: initials on green circle
        val largeIcon = buildInitialsBitmap(callerName ?: "?", 0xFF10B981.toInt())

        val builder = NotificationHelper.getBaseNotificationBuilder(
            this, NotificationHelper.CHANNEL_CALLS, NotificationHelper.GROUP_CALLS
        )
            ...
            // Always set title + text so it shows in notification shade on all devices
            .setContentTitle(callerName ?: (title ?: getString(R.string.notification_incoming_call)))
            .setContentText(getString(R.string.notification_incoming_call_subtitle))

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            // Android 12+ — beautiful native CallStyle with Answer / Decline chips
            val caller = androidx.core.app.Person.Builder()
                .setName(callerName ?: getString(R.string.notification_incoming_call))
                .setIcon(androidx.core.graphics.drawable.IconCompat.createWithBitmap(largeIcon))
                .setImportant(true)
                .build()
            builder.setStyle(
            ...
            // Pre-Android 12 — manual Answer / Decline action buttons
            builder
                .setContentTitle(callerName ?: (title ?: getString(R.string.notification_incoming_call)))
                .setContentText(getString(R.string.notification_incoming_call_subtitle))
                .addAction(
                    NotificationCompat.Action.Builder(
                        R.drawable.ic_video_call_material3,
        ...
        // Single permission check for Android 13+ (TIRAMISU)
    ...
    /**
     * Draws 1–2 letter initials centered on a solid-colour circle.
     * Used as the largeIcon for call & message notifications.
     */
    private fun buildInitialsBitmap(name: String, bgColor: Int): android.graphics.Bitmap {
        val size = 128 // px
        ...
        canvas.drawText(initials, size / 2f, size / 2f - textBounds.exactCenterY(), paint)
        return bitmap
    }
    ...
    /**
     * Helper function to download an image from a URL and convert it to a Bitmap.
     * Returns null if the URL is empty or the download fails.
     */
    private suspend fun fetchBitmapFromUrl(urlString: String?): android.graphics.Bitmap? {
        if (urlString.isNullOrEmpty() || urlString == "null") return null
        return kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
            try {
                ...
                android.util.Log.e("PushNotification", "Failed to fetch avatar image: ${e.message}")
                null
            }
        }
    }
