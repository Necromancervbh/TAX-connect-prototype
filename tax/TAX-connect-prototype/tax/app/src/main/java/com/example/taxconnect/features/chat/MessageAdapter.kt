            ...
            when (status) {
                "PENDING" -> {
                    if (isReceiver) {
                        ...
                        tvStatusMessage.text = "Pending Client Payment"
                ...
                if (layoutDocument != null) {
                    ...
                    layoutDocument.setOnClickListener {
                        try {
                            ...
                            intent.data = Uri.parse(message.imageUrl)
                            itemView.context.startActivity(intent)
                        } catch (e: Exception) {
                            val ctx = itemView.context
                            if (ctx is BaseActivity<*>) {
                                ctx.showToast(ctx.getString(R.string.cannot_open_document))
                            } else {
