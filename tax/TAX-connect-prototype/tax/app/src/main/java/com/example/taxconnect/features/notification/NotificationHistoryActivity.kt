...
class NotificationHistoryActivity : BaseActivity<ActivityNotificationHistoryBinding>() {
    ...
    inner class NotificationAdapter(private val notifications: List<NotificationModel>) :
                ...
                when (notification.type) {
                    "message" -> {
                        ...
                        if (!senderName.isNullOrEmpty()) {
                            ...
                            // If title was "New Message", we replaced it with sender name.
                            // If body is just the message, it's fine.
                        }
                    }
                    "call" -> {
                        val callerName = notification.data["callerName"]
                        if (!callerName.isNullOrEmpty()) {
                            displayTitle = "Call from $callerName"
                        }
                    }
                    "booking" -> {
                        // Keep original title usually
                    }
                }
            }

            holder.tvTitle.text = displayTitle ?: "Notification"
